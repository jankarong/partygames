...
function rollDice() {
    ...
    // Ajouter la classe d'animation de roulement
    ...
    actionDice.querySelector('.dice-text').textContent = 'Lancement...';
    whereDice.querySelector('.dice-text').textContent = 'Lancement...';
    timeDice.querySelector('.dice-text').textContent = 'Lancement...';
    resultDiv.textContent = '';

    // Attendre la fin de l'animation
    setTimeout(() => {
        // Obtenir des options aléatoires
        const action = getRandomItem(actions);
        const where = getRandomItem(wheres);
        const time = getRandomItem(times);

        // Mettre à jour le texte des dés
        ...
        resultDiv.textContent = `${action} ${where} pendant ${time}`;

        // Supprimer la classe d'animation de roulement
...
async function isPremiumUser() {
    // Utiliser le gestionnaire d'authentification global pour vérifier le statut premium
...
// Fonctions du panneau de paramètres
async function toggleSettings() {
    // Vérifier si l'utilisateur est premium avant d'ouvrir les paramètres
...
// Suivre les changements avant la sauvegarde
...
function addOption(type) {
    ...
    const defaultValue = type === 'time' ? '30 secondes' : 'Nouvelle option';
...
// Tout initialiser au chargement de la page
window.addEventListener('load', () => {
    ...
    // Ajouter un écouteur de clic à l'extérieur pour fermer les paramètres
