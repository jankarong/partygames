...
const COLOR_NAMES = {
    ...
    purple: 'Lila', orange: 'Orange', brown: 'Braun', pink: 'Rosa'
};
...
function generateUniqueSequence(length, colors) {
    ...
    if (attempts >= maxAttempts) {
        console.warn('Konnte keine eindeutige Sequenz generieren, alle Kombinationen wurden möglicherweise verwendet');
        // Clear used sequence history and restart
        previouslyUsedSequences = [];
        console.log('Sequenzverlauf gelöscht, regeneriere');
...
function renderCurrentGuess() {
    ...
    guessTitle.textContent = 'Ziehen Sie Flaschen zum Tauschen der Positionen oder klicken Sie zum Auswählen:';
    ...
    colorPalette.innerHTML = '<h4>Klicken Sie auf eine Farbe zum Auswählen:</h4>';
...
function showFeedbackPopup(feedback, isCorrect = false) {
    ...
    const title = isCorrect ? '🎉 Herzlichen Glückwunsch!' : '📊 Vermutungs-Feedback';
    ...
    popup.innerHTML = `
        ...
        <div class="feedback-details">
            <div class="feedback-item">
                <span><span class="feedback-icon">⚫</span>Richtige Position und Farbe:</span>
            ...
            <div style="margin-top: 20px; padding: 15px; background: #f0fff4; border-radius: 8px; border: 2px solid #9ae6b4;">
                <h4 style="color: #38a169; margin-bottom: 10px;">Level ${LEVELS[currentLevel].level} abgeschlossen!</h4>
                ...
                <div style="display: flex; justify-content: space-between; margin: 8px 0;">
                    <span>⏱️ Zeit:</span>
                    <strong style="color: #2d3748;">${levelTime} Sekunden</strong>
                </div>
            </div>
            ` : `
            <div style="margin-top: 15px; color: #4a5568;">
                <small>Raten Sie weiter, Sie kommen näher!</small>
            </div>
            `}
        </div>
        <button class="close-btn" onclick="closeFeedbackPopup()">
            ${isCorrect ? 'Nächstes Level' : 'Weiter'}
        </button>
    `;
...
function startLevel() {
    ...
    console.log(`Level ${level.level} gestartet, geheime Sequenz:`, secretSequence);
}
...
function submitGuess() {
    ...
    if (currentGuess.length !== level.length || !currentGuess.every(color => color)) {
        alert('Bitte vervollständigen Sie Ihre Vermutungssequenz!');
        return;
    }
