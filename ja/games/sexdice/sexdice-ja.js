// ラブダイスオンライン Game Logic - Japanese Version
const lovediceTranslations = {
...
class lovediceGame {
    constructor() {
        this.actions = this.load('diceActions', lovediceTranslations.defaultActions);
        this.wheres = this.load('diceWheres', lovediceTranslations.defaultWheres);
        this.times = this.load('diceTimes', lovediceTranslations.defaultTimes);
    ...
    roll() {
        ...
        [this.actionDice, this.whereDice, this.timeDice].forEach(d => {
            ...
            d.querySelector('.dice-text').textContent = lovediceTranslations.messages.rolling;
        });
        ...
        setTimeout(() => {
            ...
            this.resultDiv.textContent = `${w}${lovediceTranslations.messages.resultSuffix} ${a} (${t}${lovediceTranslations.messages.resultTimeSuffix})`;
    ...
    addOption(type) {
        if (type === 'action') this.actions.push(lovediceTranslations.messages.newOption);
        else if (type === 'where') this.wheres.push(lovediceTranslations.messages.newOption);
    ...
    resetToDefault() {
        this.actions = [...lovediceTranslations.defaultActions];
        this.wheres = [...lovediceTranslations.defaultWheres];
        this.times = [...lovediceTranslations.defaultTimes];
        this.save();
        this.renderAllOptions();
    }
}
