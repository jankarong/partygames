class МафияGame {
    ...
    updateRoleCounts() {
        const totalИгрокs = parseInt(document.getElementById('playerCount').value) || 0;
            ...
            <div class="role-card" data-player-id="${player.id}">
                <div class="card-inner">
                    <div class="card-front">
                        <div class="card-number">Игрок ${player.id}</div>
                    </div>
    ...
    nextNightRole() {
        ...
        const confirmДоктор = document.getElementById('confirm-doctor');
        if (doctorChoice) {
        ...
        if (confirmДоктор) {
            confirmДоктор.style.display = 'none';
        }
        ...
        Array.from(options).forEach(option => {
            option.addEventListener('click', () => {
                if (this.currentNightRole === 'detective') {
                    ...
                    const targetИгрок = this.players[targetId - 1];
    ...
    startVotingPhase() {
        ...
        votingGrid.innerHTML = this.players.map(player => `
            <div class="vote-card ${player.isAlive ? '' : 'dead'}" data-player-id="${player.id}">
                <div class="player-name">Игрок ${player.id}</div>
                <div class="player-status">${player.isAlive ? 'Click to Голос' : 'Dead'}</div>
            </div>
        `).join('');
        ...
        Array.from(voteCards).forEach(card => {
            if (!card.classList.contains('dead')) {
                card.addEventListener('click', () => this.selectГолосTarget(card));
            }
        });

        // 添加确认投票按钮的事件
        document.getElementById('confirm-vote').addEventListener('click', () => this.confirmГолос());
    }

    selectГолосTarget(card) {
        // 移除其他卡片的选中状态
    ...
    confirmГолос() {
        ...
        const votedИгрокId = parseInt(selectedCard.dataset.playerId);
        const votedИгрок = this.players.find(p => p.id === votedИгрокId);

        // 处决被投票的玩家
        votedИгрок.isAlive = false;

        // 隐藏投票界面
        document.querySelector('.voting-phase').style.display = 'none';

        // 显示投票结果
        this.showAlert(`Игрок ${votedИгрокId} has been eliminated by vote!`, () => {
            // 更新玩家状态
            this.updateИгрокStatus();
    ...
    checkGameEnd() {
        // 计算存活的玩家数量
        const aliveИгрокs = this.players.filter(p => p.isAlive);
        const aliveМафия = aliveИгрокs.filter(p => p.role === 'mafia').length;
        const aliveЖительs = aliveИгрокs.length - aliveМафия;

        let gameOver = false;
        let winner = null;

        // 如果黑手党数量大于等于好人数量，黑手党胜利
        if (aliveМафия >= aliveЖительs) {
            gameOver = true;
            winner = 'Мафия';
        }
        // 如果黑手党全部死亡，好人胜利
        else if (aliveМафия === 0) {
            gameOver = true;
            winner = 'Жительs';
        }
                ...
                <div class="card h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            ...
                            <span class="badge ${teamBadgeClass}">${role.team === 'mafia' ? 'Мафия Team' : 'Житель Team'}</span>
                        </div>
