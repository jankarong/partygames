/**
 * Komponen navigasi oyun (Versi Indonesia)
 * Menyediakan navigasi yang ditingkatkan antar oyun, tergiriş menu mengambang ve bilah akses cepat
 */

class GameNavigation {
    constructor() {
        this.games = [
            { name: 'Bola Ajaib 8', url: '/tr/games/magic8ball/magic8ball', category: 'solo' },
            { name: 'Jujur atau Berani', url: '/tr/games/TruthorDare/TruthOrDare', category: 'party' },
            { name: 'Tidak Pernah Saya', url: '/tr/games/NeverHaveIEver/NeverHaveIEver', category: 'party' },
            { name: 'Siapa yang Paling Mungkin', url: '/tr/games/WhoIsMostLikely/WhoIsMostLikely', category: 'party' },
            { name: 'Lebih Suka Mana', url: '/tr/games/WouldYouRather/WouldYouRather', category: 'party' },
            { name: 'Mafia', url: '/tr/games/mafia/mafia', category: 'strategy' },
            { name: 'Penyamaran', url: '/tr/games/undercover/undercover', category: 'strategy' },
            { name: 'Tebak Gerakan', url: '/tr/games/charades/charades', category: 'party' },
            { name: 'Tebak Kata', url: '/tr/games/hedbanzgame/hedbanzgame', category: 'party' },
            { name: 'Dadu Intim', url: '/tr/games/sexdice/sexdice', category: 'adult' },
            { name: 'Nomor Bir', url: '/tr/games/beernumber/beernumber', category: 'drinking' },
            { name: 'Piala Raja', url: '/tr/games/KingsCup/KingsCup', category: 'drinking' }
        ];
    ...
    createFloatingNav() {
        ...
        floatingNav.innerHTML = `
            ...
            <div class="nav-menu" id="navMenu">
                <h4><i class="fas fa-dice"></i> Oyun Lainnya</h4>
                ...
                <div class="nav-actions">
                    <a href="/tr/index.html">
                        <i class="fas fa-home"></i> Semua Oyun
                    </a>
                    <a href="/tr/feedback.html">
                        <i class="fas fa-comment"></i> Umpan Balik
                    </a>
                    <a href="/tr/TheList.html">
                        <i class="fas fa-list"></i> Kayıt Oyun
                    </a>
                    ...
                    <div class="auth-section">
                        <div class="auth-buttons" id="nav-auth-buttons">
                            ...
                            <a href="/register.html" class="auth-link register-link">
                                <i class="fas fa-user-plus"></i><span>Kayıt</span>
                            </a>
                        </div>
    ...
    makeLogosClickable() {
        // Keşfet logo oyun ve buat dapat diklik
    ...
    bindEvents() {
        ...
        // Lacak klik navigasi için analitik
    ...
    startPulseReminder() {
        // Tambahkan efek denyut için menarik perhatian setelah pengguna di halaman selama 30 detik
        setTimeout(() => {
            const navToggle = document.getElementById('navToggle');
    ...
    switchLanguage(lang) {
        ...
        // Oyun yang hanya tersedia dalam bahasa Inggris
        ...
        if (lang === 'zh') {
            ...
            // Jika oyun ini hanya tersedia dalam bahasa Inggris, pergi ke ana sayfa Mandarin
            if (isEnglishOnly) {
                newPath = '/zh/index.html';
            } else {
                // Hapus /de/, /fr/, ve /tr/ jika ada, lalu tambahkan /zh/
                newPath = '/zh' + currentPath.replace('/de/', '/').replace('/fr/', '/').replace('/tr/', '/');
            }
        } else if (lang === 'de') {
            ...
            // Jika oyun ini hanya tersedia dalam bahasa Inggris, pergi ke ana sayfa Jerman
            if (isEnglishOnly) {
                newPath = '/de/index.html';
            } else {
                // Hapus /zh/, /fr/, ve /tr/ jika ada, lalu tambahkan /de/
                newPath = '/de' + currentPath.replace('/zh/', '/').replace('/fr/', '/').replace('/tr/', '/');
            }
        } else if (lang === 'fr') {
            ...
            // Jika oyun ini hanya tersedia dalam bahasa Inggris, pergi ke ana sayfa Prancis
            if (isEnglishOnly) {
                newPath = '/fr/index.html';
            } else {
                // Hapus /de/, /zh/, ve /tr/ jika ada, lalu tambahkan /fr/
                newPath = '/fr' + currentPath.replace('/de/', '/').replace('/zh/', '/').replace('/tr/', '/');
            }
        } else if (lang === 'id') {
            ...
            // Jika oyun ini hanya tersedia dalam bahasa Inggris, pergi ke ana sayfa Indonesia
            if (isEnglishOnly) {
                newPath = '/tr/index.html';
            } else {
                // Hapus /de/, /zh/, ve /fr/ jika ada, lalu tambahkan /tr/
                newPath = '/id' + currentPath.replace('/de/', '/').replace('/zh/', '/').replace('/fr/', '/');
            }
        } else {
            // Beralih ke versi Inggris
            if (!currentPath.startsWith('/zh/') && !currentPath.startsWith('/de/') && !currentPath.startsWith('/fr/') && !currentPath.startsWith('/tr/')) {
    ...
    trackNavigation(gameName) {
        // Pelacakan analitik (dapat diintegrasikan ile Google Analytics atau alat lainnya)
    ...
    // Metode için menampilkan rekomendasi oyun berdasarkan oyun saat ini
    showRecommendations() {
    ...
    // Metode yang dipanggil saat oyun selesai
    onGameEnd() {
        setTimeout(() => {
    ...
    // Inisialisasi özellik autentikasi
    initAuth() {
...
// Ekspos metode için digunakan oleh oyun
