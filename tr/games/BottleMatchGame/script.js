...
const COLOR_NAMES = {
    red: 'Kirmizi', blue: 'Mavi', green: 'Yesil', yellow: 'Sari',
    purple: 'Mor', orange: 'Turuncu', brown: 'Kahverengi', pink: 'Pembe'
};
...
function renderCurrentGuess() {
    ...
    guessTitle.textContent = 'Yer degistirmek icin siseleri surukleyin veya secmek icin tiklayin:';
    ...
    colorPalette.innerHTML = '<h4>Secmek icin bir renge tiklayin:</h4>';
...
function showFeedbackPopup(feedback, isCorrect = false) {
    ...
    const title = isCorrect ? '🎉 Tebrikler!' : '📊 Tahmin Geri Bildirimi';
    ...
    popup.innerHTML = `
        ...
        <div class="feedback-details">
            <div class="feedback-item">
                <span><span class="feedback-icon">⚫</span>Dogru pozisyon ve renk:</span>
            ...
            <div style="margin-top: 20px; padding: 15px; background: #f0fff4; border-radius: 8px; border: 2px solid #9ae6b4;">
                <h4 style="color: #38a169; margin-bottom: 10px;">Seviye ${LEVELS[currentLevel].level} Tamamlandi!</h4>
                ...
                <div style="display: flex; justify-content: space-between; margin: 8px 0;">
                    <span>⏱️ Sure:</span>
                    <strong style="color: #2d3748;">${levelTime} saniye</strong>
                </div>
            </div>
            ` : `
            <div style="margin-top: 15px; color: #4a5568;">
                <small>Tahmine devam et, yaklasiyorsun!</small>
            </div>
            `}
        </div>
        <button class="close-btn" onclick="closeFeedbackPopup()">
            ${isCorrect ? 'Sonraki Seviye' : 'Devam Et'}
        </button>
    `;
...
function submitGuess() {
    ...
    if (currentGuess.length !== level.length || !currentGuess.every(color => color)) {
        alert('Lutfen tahmin dizisini tamamlayin!');
        return;
    }
