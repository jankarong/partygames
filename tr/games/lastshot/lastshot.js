class LastShotGame {
    ...
    startNewRound() {
        ...
        if (this.totalChambers > 100) {
            alert('Maksimum 100 hazneye izin verilir. Lütfen boş hazne sayısını azaltın.');
            return;
        }

        if (this.blankChambers < 1) {
            alert('En az 1 boş hazne olmalı!');
            return;
        }
        ...
        this.announce('Oyun hazır! İlk oyuncu boşluk tuşuyla tetiği çeksin.');
    }
    ...
    pullTrigger() {
        ...
        setTimeout(() => {
            if (chamber === 'blank') {
                ...
                resultText.textContent = '🔘 BOŞ! Şimdilik güvendesin.';
                ...
                resultText.textContent = `💥 MERMİ! Havuzdaki ${this.shotsInPool} kadehi iç!`;
    ...
    addShot() {
        ...
        const resultText = document.getElementById('resultText');
        resultText.textContent = `🥃 Kadeh eklendi! Havuzda şimdi ${this.shotsInPool} kadeh var.`;

        setTimeout(() => {
            if (this.gameActive && this.currentChamber < this.chambers.length) {
                resultText.textContent = 'Sıradaki oyuncunun sırası! "Tetiği Çek" butonuna bas.';
            }
        }, 2000);
    }
        ...
        if (this.totalChambers > 6) {
            cylinder.innerHTML = `
                <div class="chamber-progress">
                    <div class="progress-text">Hazne ${this.currentChamber + 1}/${this.totalChambers}</div>
    ...
    endRound() {
        ...
        if (this.shotsInPool === 0) {
            resultText.textContent = '💥 Şanslı kurtuluş! Havuzda kadeh yok. Yeni tura hazır mısın?';
        } else {
            resultText.textContent = `💥 Oyun Bitti! ${this.shotsInPool} kadeh iç ve hazır olduğunda yeni tur başlat.`;
        }
    ...
    showGameSection() {
        ...
        document.getElementById('resultText').textContent = 'Oyun hazır! İlk oyuncu "Tetiği Çek" butonuna bassın!';
        this.announce('Oyun hazır! İlk oyuncu boşluk tuşuyla tetiği çeksin.');
    ...
    startNewRoundInGame() {
        ...
        document.getElementById('resultText').textContent = 'Yeni tur başladı! İlk oyuncu "Tetiği Çek" butonuna bassın!';
        this.announce('Yeni tur başladı! Tetik için boşluk tuşuna bas.');
        document.getElementById('resultDisplay').className = 'result-display';
        document.getElementById('triggerButton').style.display = 'inline-block';
        document.getElementById('triggerButton').disabled = false;
        document.getElementById('newRoundButton').style.display = 'none';
